...
def precisionAtK(k, groundTruthList, algorithmList):
    ...
    groundTruthList = groundTruthList[:k]

    # Remove grades
    algorithmList = zip(*algorithmList)[0]
    ...
    # print(len_intersectionSet)
    # print(len_algorithmList)
...
class Algorithm(object):
    ...
    def calcSumOfSquaredErrors(self, full):
        model = self.model
        # calc vector
    ...
    def calcAvaragePrecision(self):
        ...
        for m in range(1, r_m_n.shape[0]):
            ...
            mean = self.model.mu
            b_m = self.model.b_m[m]
            algorithmList = []
            for n in groundTruthMoviesofM.tolist():
                movieIndex = n
                b_n = self.model.b_n[n]
                # calc predicated grade
                u_m = self.model.u_m[m]
                v_n = self.model.v_n[n]
                grade = np.dot(u_m, v_n) + mean + b_m + b_n
                # print(grade)
                algorithmList.append((movieIndex, str(grade) + str(movieIndex)))

            sum1_user = 0
            amount_of_movies = np.sum(groundTruthMoviesofM / groundTruthMoviesofM)
            total_amount += amount_of_movies
            for k in range(1, amount_of_movies):  # samples_in_ground_truth):
                # print(groundTruthMoviesofM)
                # print(algorithmList)
                # quit()
                ...
                print("r_k:" + str(r_k))
                r_k_minus_1 = recallAtK(k - 1, groundTruthMoviesofM, algorithmList)
                sum1_user += p_k * (r_k - r_k_minus_1)

            # print(sum1_user)
            map1 += sum1_user

        map1 /= self.model.number_of_users  # samples_in_ground_truth

        return (map1)

    def calcRootSquaredError(self, mode, full):
        ...
        for m in range(0, model.number_of_users):
            for n in range(0, model.number_of_movies):
                ...
                # print("b_n = ",b_n)
                mean = self.model.mu
                guessed = np.dot(u_m, v_n)
                if full:
                    guessed += b_m + b_n + mean
                e_m_n = -r_m_n + guessed
                errors_matrix[m, n] = e_m_n
                size_of_set += 1
...
class GradientDescent(Algorithm):
    def LearnModelFromDataUsingSGD(self):
        ...
        for epoch in range(0, self.hyperparameters.epochs_limit):
            ...
            e1_test = self.calcRootSquaredError('test', True)
            e1_training = self.calcRootSquaredError('training', True)
...
class AlternatingLeastSquares(Algorithm):
    def LearnModelFromDataUsingALS(self):
        ...
        loss = 1000000000
        iters_limit = hyperparameters.epochs_limit
        calcRootSquaredErrorVector_test = []
        calcRootSquaredErrorVector_training = []
        ...
        while loss > delta and iters_counter < iters_limit:
            ...
            e1_test = self.calcRootSquaredError('test', False)
            e1_training = self.calcRootSquaredError('training', False)
            calcRootSquaredErrorVector_test.append(e1_test)
            calcRootSquaredErrorVector_training.append(e1_training)
            print("e1_test: " + str(e1_test))
            print("e1_training: " + str(e1_training))
            ...
            for n in self.model.n_list:
                ...
                for m in self.model.n_list[n]:
                    ...
                    b_m = model.b_m[m]
                    b_n = model.b_n[n]
                # b_n += (r_m_n - (mean -np.outer(u_m,v_n) -b_m))/ (1 + lamda_b_v)
                ...
                for m in self.model.n_list[n]:
                    ...
                    u_m = model.u_m[m]
                    sum_b += r_m_n * u_m
                # sum_b -= mean#lamda_b_u*b_m + lamda_b_v*b_n + mean
            ...
            for m in self.model.m_list:
                ...
                # b_m += (r_m_n -(mean -np.outer(u_m,v_n) -b_m))/ (1 + lamda_b_u)
                # sum_a += mean#lamda_b_u*b_m + lamda_b_v*b_n + mean
...
def getUserHistoryAndPredictions(userIndex, h, dataSet, model):
    ...
    predecited_list = []
    # for index in indexes_of_existing_movies:
    for index in range(0, model.number_of_movies):
        u_m = model.u_m[userIndex]
        v_n = model.v_n[index]
        predecited_grade = np.dot(u_m, v_n)  # +model.mu#+ model.b_m[m] + model.b_n[n] + model.mu
    ...
    h_counter = 0
    for predection in sorted_predecited_list:
        predection_index = predection[0]
        if not predection_index in dataSet.movies.dict:
            continue
        movie = dataSet.movies.dict[predection_index]
        movieName = movie['Title']
        movieIndex = movie['MovieID']
        if movieName not in shownMovies:
...
def delivarble2():
    ...
    calcRootSquaredErrorVectorVector = []
    calcSumOfSquaredErrorsVector = []
    ...
    for lamda in lamda_range:
        ...
        hyperParamtersDict['epochs_limit'] = 1
        ...
        sum_of_squared_errors = gr.calcSumOfSquaredErrors(True)

        calcRootSquaredErrorVectorVector.append(calcRootSquaredErrorVector_test[-1])
        calcSumOfSquaredErrorsVector.append(sum_of_squared_errors)
    ...
    x_a = lamda_range
    y_a = calcRootSquaredErrorVectorVector
    x_label = 'lamda'
    y_label = 'Root Squared Error'
    colorA = 'b'
    graph_label = 'Root Squared Error over Lamda'
    ...
    x_a = lamda_range
    y_a = calcSumOfSquaredErrorsVector
    x_label = 'lamda'
    y_label = 'Sum of Squared'
    colorA = 'b'
    graph_label = 'Sum of Squared Errors over Lamda'
    box_string = hyperparameters.str1()
    oneStreamsGraph(x_a, y_a, x_label, y_label, colorA, graph_label, box_string, logX=True)
...
def delivarble3():
    ...
    for attributes_number in attributes_range:
        ...
        hyperParamtersDict['epochs_limit'] = 10
        hyperParamtersDict['min_loss_limit'] = 0.1
...
def delivarble4():
    ...
    for attributes_number in attributes_range:
        ...
        durationVector.append(duration)
    ...
    colorA = 'b'
    graph_label = 'Duration over attributes'
    for key in hyperparameters.dict1:
        if key.count('attributes'):
            del hyperparameters.dict1[key]
...
def delivarble5():
    ...
    hyperParamtersDict['alpha'] = 0.01
    hyperParamtersDict['number_of_attributes'] = 10
    hyperParamtersDict['epochs_limit'] = 4
    hyperParamtersDict['min_loss_limit'] = 0.1
    dataSet, model, hyperparameters = prepareStructers(moviesFile, ratingsFile, usersFile, hyperParamtersDict)
    # gr = GradientDescent(dataSet,model,hyperparameters,"gd_mean.csv")
    # gr.LearnModelFromDataUsingSGD()
    ...
    r_m_n = model.r_m_n_training
    # get list of movies of that user
    for counter in range(0, model.number_of_users):
        movies_of_user = r_m_n[counter]
        # print(np.where(movies_of_user>0)[0])
        indexes_of_existing_movies = np.where(movies_of_user > 0)[0]
        if len(indexes_of_existing_movies) >= 3:
            print("For user number " + str(counter) + ":")
            getUserHistoryAndPredictions(counter, 5, dataSet, model)  # Display 5 top recommandations
        if counter >= 5:  # Show no more than 5 users
            break
...
def delivarable1_gradientDecsent():
    ...
    calcRootSquaredErrorVector_test, calcRootSquaredErrorVector_training = gr.LearnModelFromDataUsingSGD()
    box_string = hyperparameters.str1()
    # delivarble1Graph(calcRootSquaredErrorVector_test,calcRootSquaredErrorVector_training,'Gradient Descent',box_string)
    delivarble1Graph(calcRootSquaredErrorVector_test, calcRootSquaredErrorVector_training,
                     'Root Squared Error over Iterations (GD)', box_string)
...
def delivarable1_ALS():
    ...
    box_string = hyperparameters.str1()
    delivarble1Graph(calcRootSquaredErrorVector_test, calcRootSquaredErrorVector_training,
                     'Root Squared Error over Iterations (ALS)', box_string)
